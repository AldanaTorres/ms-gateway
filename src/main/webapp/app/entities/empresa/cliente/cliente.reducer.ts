...
import { ICliente, defaultValue } from 'app/shared/model/empresa/cliente.model';
...
const apiUrl = 'services/empresa/api/clientes';

// Actions

export const getEntities: ICrudGetAllAction<ICliente> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_CLIENTE_LIST,
  payload: axios.get<ICliente>(`${apiUrl}?cacheBuster=${new Date().getTime()}`),
});
